...
int zi = 0;
    ...
    public static class Debug {
        ...
        public static void printlong10(String tag, Main.Curve25519.long10 value) {
            if (!Enable)
                return;
        ...
        public static void printBytes(String tag, byte[] bytes) {
            ...
            Output.printBytes(tag, bytes);
       }
    }

    public static void GenerateSignTestCases() throws Exception {
	    ...
	    for (String s1 : secrets) {
	        for (String s2 : secrets) {
                ...
                byte[] h = algorithm.digest(s1.getBytes("UTF-8"));
                byte[] x = algorithm.digest(s2.getBytes("UTF-8"));

                byte[] v = new byte[32];
                byte[] s = new byte[32];
                rand.nextBytes(s);

                if (!Curve25519.sign(v, h, x, s))
                    throw new Exception("Failed processing (" + s1 + "," + s2 + ")");

                Output.printBytes("v", v);
                Output.printBytes("h", h);
                Output.printBytes("x", x);
                Output.printBytes("s", s);
            }
        }
    }
    ...
    public static void GenerateKeygenTestCases() {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            Output.printBytes("k_in", k);

            Curve25519.keygen(P, s ,k);

            Output.printBytes("P", P);
            Output.printBytes("s", s);
            Output.printBytes("k_out", k);
        }
    }
    ...
    public static void GenerateVerifyTestCases() {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            Curve25519.verify(Y, v, h, P);

            Output.printBytes("Y", Y);
            Output.printBytes("v", v);
            Output.printBytes("h", h);
            Output.printBytes("P", P);
        }
